...
function trimAudio(inputPath, outputPath, seconds = 15) {
...
function unwrapMessage(m) {
    // Unwrap viewOnce/status wrappers used by WhatsApp statuses
...
async function getMediaBuffer(message, type) {
    try {
        ...
        const m = unwrapMessage(raw); // unwrap status/viewOnce wrappers
        ...
        switch (type) {
            case 'audio':
                if (m.audioMessage) {
                    ...
                    messageType = m.pttMessage;
                    fileExt = '.ogg';
                    downloadType = 'audio';
                    debugLog('Found ptt message');
                }
                break;
            case 'video':
                if (m.videoMessage) {
...
function extractQuotedMessage(message) {
    ...
    // extendedTextMessage (replying to any message)
    if (msg.extendedTextMessage?.contextInfo?.quotedMessage)
        return msg.extendedTextMessage.contextInfo.quotedMessage;
    // direct media messages that have contextInfo (e.g. replying while sending media)
    ...
    if (msg.audioMessage?.contextInfo?.quotedMessage)
        return msg.audioMessage.contextInfo.quotedMessage;
    // viewOnce wrappers
    ...
    if (inner?.extendedTextMessage?.contextInfo?.quotedMessage)
        return inner.extendedTextMessage.contextInfo.quotedMessage;
    return null;
}
...
async function shazamCommand(sock, chatId, message) {
    ...
    try {
        ...
        // Trim to 15 seconds so the Shazam API doesn't reject large files
        let uploadPath = tempPath;
        trimmedPath = path.join(tempDir, `${Date.now()}_trimmed.mp3`);
        try {
            await trimAudio(tempPath, trimmedPath, 15);
            uploadPath = trimmedPath;
            debugLog('Audio trimmed to 15s:', { path: trimmedPath });
        } catch (trimErr) {
            debugLog('Trim failed (will upload original):', trimErr.message);
        }

        let mediaUrl = '';
        try {
            debugLog('Uploading to Uguu...');
            const res = await UploadFileUgu(uploadPath);
        ...
        for (const p of [tempPath, trimmedPath]) {
