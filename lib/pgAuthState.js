    ...
    const state = {
        ...
        keys: {
            get: async (type, ids) => {
                ...
                try {
                    const fullKeys = keys.map(k => `${APP_PREFIX}:${k}`);
                    const { rows } = await pool.query(
                        'SELECT key, value FROM baileys_auth WHERE key = ANY($1)', [fullKeys]
                    );
                    const data = {};
                    for (const row of rows) {
                        const keyWithoutPrefix = row.key.slice(APP_PREFIX.length + 1);
