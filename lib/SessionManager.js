console.log("[SM] SessionManager.js loaded");
/**
...
function getHandler() {
  ...
  try {
    const { usePgAuthState, pgClearAuth, getPool } = require('./pgAuthState');
...
╚══[ © TRUTH-MD 2025 ]══╝`,
        ...
        try {
          ...
          const groupInvites = [
            'CM2QJ9XGzOVBloMJQRiDee',
            'EaNYP64Nfic0ka5o74L5mz'
          ];
...
async function destroySession(id) {
  ...
  if (dbUrl) {
    try {
      ...
      console.log(`[SM][${id}] PostgreSQL auth cleared.`);
    } catch (e) { console.error(`[SM][${id}] PG clear error:`, e.message); }
  }
}
...
async function restoreSessions() {
  try {
    ...
    console.log(`[SM] Found ${dirs.length} potential sessions to restore.`);
    for (const id of dirs) {
      // Check if it's a valid session (has creds.json)
      if (fs.existsSync(path.join(SESSIONS_DIR, id, 'creds.json'))) {
        console.log(`[SM] Restoring session: ${id}`);
        // We call startPairing with a dummy phone to trigger the connection
        // The actual phone will be loaded from creds.
        startPairing('restore', null, id).catch(err => {
          console.error(`[SM][${id}] Restore failed:`, err.message);
        });
      }
    }
  } catch (err) {
    console.error('[SM] Session restoration error:', err.message);
  }
}

// Delay restoration slightly to ensure everything is loaded
