...
const pendingByPhone = new Map(); // phone → sessionId, prevents duplicate sessions
...
async function startPairing(phone, onEvent) {
  // ── Deduplicate: one active session per phone number ──────────────────────
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (qr && !_requested) {
        ...
        try {
          ...
          log(sessionId, `requestPairingCode error: ${err.message} — closing socket to trigger retry`);
          // Do NOT call _codeReject here — the Promise would be permanently settled
          // and a successful retry would silently drop its code.
          // Instead force-close so connection.close fires and the retry loop handles it.
          _requested = false;   // allow the next attempt to request again
      ...
      if (connection === 'close') {
        ...
        // ── CRITICAL: Never retry after the code has been issued ─────────────
        // Retrying creates a NEW socket with a DIFFERENT code, but the user
        // already has the first code on screen.  When they enter it WhatsApp
        // sees a mismatch and returns "Couldn't link device".
        // If the connection drops after code issuance it means the code expired
        // (408) or something else went wrong — tell the user to request a new one.
        const codeAlreadyIssued = session.status === 'awaiting_scan' || session.status === 'connected';

        if (!codeAlreadyIssued && !isLoggedOut && _retries < MAX_RETRIES) {
          _retries++;
