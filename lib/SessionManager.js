...
async function startPairing(phone, onEvent, existingId = null) {
  ...
  async function attempt(preserveCredentials = false) {
    ...
    if (dbUrl) {
      console.log(`[SM][${sessionId}] Using PostgreSQL for session persistence (Phone: ${phoneKey}).`);
      const pgAuth = await usePgAuthState(dbUrl, phoneKey);
      state = pgAuth.state;
    ...
    const sock = makeWASocket({
      ...
      logger,
      browser: ['Mac OS', 'Chrome', '131.0.6778.205'],
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (qr && !_requested) {
        ...
        try {
          ...
          log(sessionId, `QR ready — requesting pairing code for ${phoneKey} with prefix ${prefix}`);

          const raw  = await sock.requestPairingCode(phoneKey, prefix);
          log(sessionId, `Raw pairing code response received from WhatsApp`);
          const code = raw?.match(/.{1,4}/g)?.join('-') || raw;

          session.pairingCode = code;
          session.status      = 'awaiting_scan';
          log(sessionId, `Pairing code issued: ${code}. Please check your phone for a notification.`);
          _codeResolve(code);
        } catch (err) {
...
╚══[ © TRUTH-MD 2025 ]══╝`,
        ...
        // FIXED: Re-enabled. server.js (Heroku entry point) does NOT use MultiInstanceManager.
        // Sessions from SessionManager must handle their own messages or commands never fire.
        sock.ev.on('messages.upsert', async ({ messages, type }) => {
          // 'notify' = inbound messages, 'append' = own messages / self-chat
