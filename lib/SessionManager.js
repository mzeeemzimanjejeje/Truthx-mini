  ...
  async function attempt(preserveCredentials = false) {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        // ── After code issued: try to reconnect (preserving creds) ────────
        // 515 means WhatsApp asked us to restart — reconnect so the socket is
        // ready when the user enters the code.
        // 408 means timeout — give one reconnect attempt before giving up.
        if (_codeIssued && session.status !== 'connected' && !isLoggedOut) {
          if (_reconnects < MAX_RECONNECTS) {
            _reconnects++;
            log(sessionId, `Reconnecting (${_reconnects}/${MAX_RECONNECTS}) after code issued (${statusCode}) — preserving session`);
            await delay(2000);
            return attempt(true);   // keep credentials, skip re-requesting
          }
          // Exhausted reconnects after code issued — code is expired
