...
async function createSession(phone, sessionId) {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      // Any disconnect = clean up immediately, no reconnect
      // Users pair once; if they want the bot back, they pair again
      const code = lastDisconnect?.error?.output?.statusCode;
      const reason = code === DisconnectReason.loggedOut || code === 401
        ? 'logged out'
        : `disconnected (code ${code || 'unknown'})`;

      log(sessionId, `Session ${reason} — clearing session data`);
...
module.exports = { createSession, destroySession, allSessions, sessionInfo, setIO };
