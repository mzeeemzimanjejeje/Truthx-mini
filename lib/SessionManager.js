...
// ─── Main entry point ─────────────────────────────────────────────────────────
// Starts a WhatsApp session and waits until the pairing code is ready.
// Returns { sessionId, code } — safe to send directly in the HTTP response.
// The socket keeps running in the background after this resolves.
async function startPairing(phone) {
  ...
  const sock = makeWASocket({
    ...
    logger,
    // Identify as Chrome so WhatsApp treats this like a real Chrome browser session
    browser: Browsers.chrome('TRUTH-MD'),
  ...
  // Wait 2s then request the pairing code — resolve once we have it
  ...
  // Attach event listeners — these run in the background after we return
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      const reason = statusCode === DisconnectReason.loggedOut || statusCode === 401
        ? 'logged out'
        : `closed (code ${statusCode || 'unknown'})`;
