...
// In-memory antilink cache — survives within the dyno session, avoids disk read
// on every incoming group message. Seeded from file on first access.
let _antilinkMem = null;
function _ensureAntilinkMem() {
    if (_antilinkMem === null) {
        try { _antilinkMem = loadJSON(ANTILINK_FILE, {}); } catch (_) { _antilinkMem = {}; }
    }
    return _antilinkMem;
}
