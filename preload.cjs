...
function onSIGTERM() {
  // SIGTERM is a normal panel/system restart signal — do NOT exit with 143
  // Let the gracefulShutdown in index.js run first (it saves session & exits 0)
  // Fallback: if nothing exits within 8s, exit cleanly ourselves
  console.log('[TRUTH-MD] SIGTERM received — waiting for graceful shutdown...');
  setTimeout(() => {
    console.log('[TRUTH-MD] Graceful shutdown timeout — exiting cleanly');
    _origExit(0);
  }, 8000).unref();
}
